...
export const TicketRaiser: React.FC<TicketRaiserProps> = ({ appClientId, apiEndpoint, applicationName }) => {
        ...
        if (canvas) {
            ...
            if (ctx) {
                ...
                if (!isStarted) {
                    ...
                    ctx.moveTo(x, y); 
                    setIsStarted(true);  // Set the drawing start flag
                } else {
    ...
    const undoLastAction = () => {
        if (history.length > 1) { 
            // Undo to the previous state if more than one state exists
            const canvas = canvasRef.current;
            const lastState = history[history.length - 2]; // Second-to-last state
            ...
            // Clear the canvas if this is the last state
            const canvas = canvasRef.current;
            ...
            console.log("No more actions to undo."); // No history left
        }
    };
